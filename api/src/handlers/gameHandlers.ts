...
export default (io: Server, socket: Socket) => {
  ...
  const joinGame = async (gameId: Types.ObjectId, playerName: string) => {
    if (!gameId) {
      return socket.emit(
        "error",
    ...
    game.players.push(user._id as unknown as string);
  
    await game.save();
    socket.join(gameId.toString());
    io.to(gameId.toString()).emit("player-joined", gameId, user._id);
  };
  
  const getGameStatus = async (gameId: Types.ObjectId,) => {
    const game = await Game.findOne({ _id: gameId });
    if (!game) {
      return socket.emit("error", "Game not found");
    }
